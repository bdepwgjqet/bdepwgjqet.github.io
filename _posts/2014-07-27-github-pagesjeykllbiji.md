...
- **_includes**：用来存放需要被反复调用的文件，可以通过 **\{\% include file.html \%\}**（不包括反斜杠）来调用flie.html文件。
...
以上 __\{\% highlight java linenos \%\}__（不包括反斜杠）中java表示语言，linenos表示代码框中显示行号。使用default样式生成的css不支持linenos。
